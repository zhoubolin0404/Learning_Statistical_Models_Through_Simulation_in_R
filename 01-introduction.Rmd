...
```{r chapter-status, echo=FALSE, warning=FALSE, results='asis'}
...
```{r basic-glm, echo=FALSE, fig.cap="线性模型Y = 3 + 2X的模拟数据", message=FALSE, warning=FALSE}
...
```{r glm, webex.hide="查看绘制该图的R代码", ref.label="basic-glm", eval=FALSE}
```

你可能会发现上述方程表示的是一条直线($y = mx + b$)，其中$\beta_0$是截距，$\beta_1$是斜率。$e_i$是样本$i$的模型误差，表示样本观测值$Y_i$与给定$X_i$的模型预测值之间的差距。

<div style="border: 2px solid orange; padding: 10px; background-color: #fff5cc;">
**表示法惯例**
...
<br>
<div style="border: 2px solid blue; padding: 10px; background-color: #e6f7ff;"> 
**线性模型中的"线性"并不像你想象的那样!**
...
```{r tryptych, echo=FALSE, fig.cap="用线性模型建模的非线性关系", fig.height = 3, fig.width = 8}
## quadratic
